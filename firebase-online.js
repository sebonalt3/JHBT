(function () {
  ...
  async function init(handlers) {
    ...
    try {
      ...
      if (auth.currentUser) {
        ...
        else backupWriteBlocked = false;
        status(`온라인 연결됨 · UID ${code()}`);
        publish().catch(() => {});
      } else {
        backupWriteBlocked = false;
  ...
  async function createAccount(password) {
    ...
    if (backupWriteBlocked) throw new Error('온라인 초기화가 끝나지 않았습니다. 연결 상태를 확인하세요.');
  ...
  async function login(privateCode, password) {
    ...
    // 계정을 바꾸기 전 현재 계정의 예약/진행 중인 백업을 끝낸다.
    if (auth.currentUser && !backupWriteBlocked) {
  ...
  async function restoreCurrent() {
    ...
    if (!snapshot.exists()) { backupWriteBlocked = false; return false; }
  ...
  async function saveNow() {
    ...
    if (backupWriteBlocked) throw new Error('온라인 백업을 불러오기 전에는 저장할 수 없습니다.');
    clearTimeout(saveTimer);
    saveTimer = undefined;
  ...
  window.JihooOnline = { init, code, createAccount, login, recoverAccount: login, changePassword, restoreCurrent, saveNow, queueSave, publish, leaderboard,
    connected: () => !!(auth && auth.currentUser), enabled: () => !!auth && !backupWriteBlocked };
})();
